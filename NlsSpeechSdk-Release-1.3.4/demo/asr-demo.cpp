...
void* pthreadFunc(void* arg) {
    ...
    */
	NlsRequest* request = NlsClient::getInstance()->createAsrRequest(callback, tst->config.c_str());
	/***********************************************************/

	/*****************以参数设置方式创建request******************
    NlsRequest* request = NlsClient::getInstance()->createAsrRequest(gCallback, NULL);
    ...
    ************************************************************/
	if (request == NULL) {
		cout << "createAsrRequest fail." << endl;
 ...
 * 识别单个音频数据
 */
int speechAsrFile(const char* configFile, const char* id, const char* scret) {
...
int speechAsrMultFile(const char* configFile, const char* id, const char* scret) {
...
int main(int arc, char* argv[]) {
	...
	int ret = NlsClient::getInstance()->setLogConfig("log-asr.txt", LogInfo);
	if (-1 == ret) {
	...
	/*识别单个音频数据*/
	speechAsrFile(argv[1], argv[2], argv[3]);

	/*识别多个音频数据*/
	//speechAsrMultFile(argv[1], argv[2], argv[3]);
